...
df <- fread(paste0(path, "k562-locations_percentages_for-plot_24042024.txt"))
stat.test <- fread(paste0(path, "k562-locations_statistics_for-plot_24042024.txt"))

# STEP 2: prepare the order of the locations
levs <- c("E123 Active enhancer", "E123 Promoter", 
          "K562 DHS broadpeak","K562 DHS narrowpeak")

df$Annotation= factor(df$Annotation, levels = levs)
df$Group = factor(df$Group, levels = c("K562 raQTLs", "K562 controls", "all SuRE SNPs"))
...
#stat.test$Annotation <- (as.numeric(x,1:6))-1

stat.test1 <- stat.test[stat.test$group2 == "K562 controls", ]
...
p <- ggplot(df, aes(x=Annotation, y=Percentage)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(df$Percentage + 7))) +
  geom_bracket(
...
ggsave(paste0(path, "locations/k562_locations_freeze7_Routput_24042024.pdf"), p, units="mm", height=120, width=150)
