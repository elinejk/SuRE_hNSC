...
ov_lab <- df[, c("Tissue", "Set", "Significant eQTLs")] %>% group_by(Set) %>% top_n(3) %>% ungroup()
conc_lab <- df[, c("Tissue", "Set", "Concordance")] %>% group_by(Set) %>% top_n(3) %>% ungroup()
...
pl <- ggplot(df, aes(x=Tissue,y=Set)) +
  ...
  geom_text(data = ov_lab, aes(x=Tissue,y=Set,label=round(`Significant eQTLs`, 1)), size = 4, nudge_y = 0.5) +
  geom_text(data = conc_lab, aes(x=Tissue,y=Set,label=round(Concordance, 1), fontface = "italic"), size = 4, nudge_y = 0.3) +
  theme_minimal() +
...
ggsave('V:/ddata/CELB/poot/Eline Koornstra/SuRE_general/Results/freeze7/figures/eQTLs/hnsc-raqtls_gtex-v7_overlap_10042024_wtop3labels.pdf', pl,
