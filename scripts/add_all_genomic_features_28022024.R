...
df_range <- GRanges(seqnames = Rle(df$chrom), 
                    ranges=IRanges(start=df$pos_hg19,width =1)) 
...
tissue <- "E009"
#chromhmm <- fread(paste0(an_path, "chromHMM/E009_15_coreMarks_dense.bed.gz"))
chromhmm <- fread(paste0(an_path, "chromHMM/E009_25_imputed12marks_dense.bed.gz"), skip=1)
...
# broadpeak
n <- fread(paste0(an_path, "DHS/ENCFF818JZS_DHS_2018_hg19_NPC.bed"))
...
df_range$NPC_broadpeak_DHS_dist <- NA
df_range$NPC_broadpeak_DHS_dist[queryHits(closest)] <- mcols(closest)$distance

head(df_range)

# narrowpeak
n <- fread(paste0(an_path, "DHS/hNPC_hg19_2020_DHS.bed"))
names(n) <- c("chr", "start", "end", "V4", "V5", "V6", "V7", "V8", "V9", "V10")
n[, chr := sub("chr", "", chr)]
nr <- GRanges(seqnames=Rle(n$chr), ranges = IRanges(start=n$start, end=n$end))

closest <- distanceToNearest(df_range, subject = nr)
df_range$NPC_narrowpeak_DHS_dist <- NA
df_range$NPC_narrowpeak_DHS_dist[queryHits(closest)] <- mcols(closest)$distance
...
dt <- dt %>%
  ...
  mutate(E009_promoter = ifelse(base::grepl("TSS_Promoter",dt$chromHMM_region), "1", "0")) %>%
  mutate(E009_enhancer = ifelse(base::grepl("Active_Enhancer",dt$chromHMM_region), "1", "0")) %>%
  mutate(NPC_broadpeak_DHS = ifelse(NPC_broadpeak_DHS_dist == 0, "1", "0")) %>%
  mutate(NPC_narrowpeak_DHS = ifelse(NPC_narrowpeak_DHS_dist == 0, "1", "0"))
...
newcols <- c("SNP_ID", "UCSC_prom_dist", "UCSC_promoter", "Hoffman_enh_dist", "Hoffman_enhancer", "GENCODE_location", 
		...
		"chromHMM_mark", "chromHMM_region", "E009_promoter", "E009_enhancer", 
		"NPC_broadpeak_DHS_dist", "NPC_broadpeak_DHS", "NPC_narrowpeak_DHS_dist", "NPC_narrowpeak_DHS")
...
# dhs
b <- dt[dt$NPC_broadpeak_DHS == "1", ]
print(paste0("number of SNPs in broadpeak DHS: ", nrow(b)))
b <- dt[dt$NPC_narrowpeak_DHS == "1", ]
