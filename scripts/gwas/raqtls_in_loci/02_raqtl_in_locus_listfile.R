...
for (pheno in phenos) {
  ...
  df <- df[df$hNSC_raQTL == "1",]
  df$phenotype <- pheno
  
  df_coord <- merge(df, raqtls, by = "SNP_ID")
  
  cols <- c("SNP_ID", "chrom", "pos.hg19", "locus", "chr", "start", "end","GWAS_SNP", "GWAS_P","hNSCraQTL_GWAScandidate", "phenotype")
  a <- select(df_coord, all_of(cols))
  #names(a) <-  c("SNP_ID", "chrom", "pos.hg19", "locus", "chr", "start", "end","GWAS_SNP", "GWAS_P","hNSC-raQTL_GWAS-candidate", "phenotype")
