...
### parameters
phenos <- c("ADHD", "ASD", "BPD", "CP", "IQ", "MDD", "SCZ", "allphenos", "allphenos-shortestdist")
sets <- c("controls", "sampled_matched_controls", "matched_controls", "all_sure")
...
for (pheno in phenos){
	...
	for (set in sets) {
        ...
        pl <- ggplot(data = kb, aes(x=distance_lead, y = after_stat(density))) +
	     ...
	     ggsave(paste0(path, "figures/raqtls_vs_", set, "/", pheno, "_gwassign_snp-to-lead_raqtl_vs_", set, "_histo_17042024.pdf"), pl, units="mm",width=200, height = 250)
