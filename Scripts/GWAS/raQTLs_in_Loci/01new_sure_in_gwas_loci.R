...
## STEP 1.3: prepare hNSC raQTLs and gwas sign snps
...
# sure snps in locus that are raQTLs
inlocus <- inlocus %>% mutate(hNSC_raQTL = ifelse(inlocus$SNP_ID %in% raqtl$SNP_ID, "1", "0"))
inlocus$hNSC_raQTL <- as.numeric(inlocus$hNSC_raQTL)

# raQTLs in the locus that were part of the gwas sumstats (sure in locus vs raqtls vs gwas snps)
inlocus <- inlocus %>% mutate(hNSC_raQTL_GWAS = ifelse(inlocus$SNP_ID %in% raqtl$SNP_ID & inlocus$SNP_ID %in% gwas$SNP, "1", "0"))
inlocus$hNSC_raQTL_GWAS <- as.numeric(inlocus$hNSC_raQTL_GWAS)
...
# raQTLs are also "in LD with lead SNPs/indsig" locus snps
inlocus <- inlocus %>% mutate(hNSCraQTL_GWAScandidate = ifelse(inlocus$SNP_ID %in% snps$rsID & inlocus$SNP_ID %in% raqtl$SNP_ID, "1", "0"))
inlocus$hNSCraQTL_GWAScandidate <- as.numeric(inlocus$hNSCraQTL_GWAScandidate)
...
b <- loc %>% 
  group_by(loc$locus) %>% 
  summarize(nBoth = sum(GWAS_SNP), nhNSC_raQTL = sum(hNSC_raQTL), nhNSC_raQTL_GWAS = sum(hNSC_raQTL_GWAS),
            nSuRE_nSNPs = sum(SuRE_GWAScandidate), nhNSC_raQTL_nSNPs = sum(hNSCraQTL_GWAScandidate), nGWASsign_SuRE = sum(GWAS_sign))
names(b) <- c("GenomicLocus", "nBoth", "nhNSC_raQTL", "nhNSC_raQTL_GWAS", "nSuRE_nSNPs", "nhNSC_raQTL_nSNPs", "nGWASsign_SuRE")
...
subfile <- fullfile[,c("GenomicLocus", "CHR", "start", "end", "LeadSNPs", "nGWAS", "nSNPs", 
                       "nSuRE", "nBoth", "nSuRE_nSNPs", "nGWASsign_SuRE", "nhNSC_raQTL", "nhNSC_raQTL_GWAS", "nhNSC_raQTL_nSNPs")]
